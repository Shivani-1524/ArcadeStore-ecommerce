...
const CartSummary = () => {
    ...
    const getSavedAmt = () => cartItems.reduce((prev, acc) => {
        return prev += acc.originalprice ? (acc.originalprice - acc.currentprice) : 0
    }, 0)
    const getTotalAmt = () => cartItems.reduce((prev, acc) => {
        return prev += acc.qty ? acc.currentprice * acc.qty : acc.currentprice * 0
    }, 0)
    ...
    return (
        <div className="buy-container">
            <div className="cart-details">
                ...
                {cartItems.map((item, index) => <div key={index} className="item-detail">
                    <p className="sm-title n-wt">{item.title} x{item.qty}</p>
                    <p className="sm-title n-wt">Rs. {item.qty ? item.currentprice * item.qty : item.currentprice * 0 }</p>
                </div>)}
